...
function QuizPhase({
  ...
  const t = useTranslations('OnboardingToets')
    ...
    <div className="space-y-6">
      ...
      <div className="divide-y divide-border rounded-xl border border-border overflow-hidden">
        {results.map((r) => (
          <div
            key={r.questionId}
            className={cn('flex gap-4 px-5 py-4', r.correct ? 'bg-surface' : 'bg-accent-2-light')}
          >
            <span
              className={cn('mt-0.5 shrink-0 text-lg font-bold', r.correct ? 'text-accent' : 'text-accent-2')}
              aria-label={r.correct ? t('correct') : t('wrong')}
            >
              {r.correct ? '✓' : '✗'}
            </span>
                ...
                <div className="mt-2 space-y-1 text-sm">
                  ...
                  <p className="text-text-muted">
                    ...
                    <span className="font-mono text-accent">
                      {r.latexAnswer ? <TeX tex={r.latexAnswer} /> : <span className="font-mono">{r.correctAnswer}</span>}
                    </span>
                  </p>
                </div>
              )}
...
function PadPhase({
  ...
  const initialSel = useMemo(() => {
    ...
    for (const topic of sorted) {
      if (diagnosticTopicIds.has(topic.id)) {
        const correct = correctTopicIds.has(topic.id)
        m.set(topic.id, { ken: correct, wil: !correct })
      } else {
        m.set(topic.id, { ken: false, wil: true })
      }
    }
    return m
  ...
  function submit() {
    ...
    setError(null)
    const topicIds = sorted.map((topic) => topic.id)
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-text-muted">{t('padIntro')}</p>
      </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-border">
        <table className="w-full min-w-[28rem] border-collapse text-sm">
          ...
          <tbody>
            {sorted.map((topic, i) => {
              const c = sel.get(topic.id) ?? { ken: false, wil: false }
              const inDiagnostic = diagnosticTopicIds.has(topic.id)
              return (
                <tr
                  key={topic.id}
                  className={cn(
                  ...
                  <td className="px-4 py-3">
                    ...
                    {inDiagnostic && (
                      <span
                        className={cn(
                          'ml-2 text-xs',
                          correctTopicIds.has(topic.id)
                            ? 'text-accent'
                            : 'text-accent-2',
                        )}
                      >
                        {correctTopicIds.has(topic.id) ? `✓ ${t('correct').toLowerCase()}` : `✗ ${t('wrong').toLowerCase()}`}
                      </span>
                    )}
                  </td>
                  <td className="px-4 py-3">
                    <input
                      ...
                      checked={c.ken}
                      onChange={(e) => setCell(topic.id, 'ken', e.target.checked)}
                      aria-label={`${topic.title}: ${t('colKnow').toLowerCase()}`}
                    />
                  </td>
                  <td className="px-4 py-3">
                    <input
                      ...
                      checked={c.wil}
                      disabled={c.ken}
                      onChange={(e) => setCell(topic.id, 'wil', e.target.checked)}
                      aria-label={`${topic.title}: ${t('colPractice').toLowerCase()}`}
                    />
                  </td>
                </tr>
              )
